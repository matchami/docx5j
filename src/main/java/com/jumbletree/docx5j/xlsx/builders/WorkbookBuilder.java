...
public class WorkbookBuilder implements BuilderMethods {
    ...
    Long createBorder(STBorderStyle topStyle, Color topColor, STBorderStyle rightStyle, Color rightColor,
            STBorderStyle bottomStyle, Color bottomColor, STBorderStyle leftStyle, Color leftColor) {
        ObjectKey key = new ObjectKey(topStyle, topColor, rightStyle, rightColor, bottomStyle, bottomColor, leftStyle,
                leftColor);
    ...
    protected void applyFont(String fontName, int size, Color color, boolean bold, boolean italic,
            STUnderlineValues underline, List<JAXBElement<?>> fontProperties) {
    ...
    /**
     * 
     * @param series a list of name, range pairs that will make up the chart eg
     *               createChart("First series", "Sheet1!$B$1:$B$20");
     * @throws Docx4JException
     */
    ...
    /**
     * Sets the sheet's name. Called by the sheet's factory, rather than by the user
     * directly. Use getSheet(index).setName(name) instead.
     */
    ...
    public void createComment(WorksheetBuilder worksheet, Cell cell, String author, String comment,
            CommentPosition position) throws Docx4JException {
        ...
        if (this.comments.size() <= sheetIndex || this.comments.get(sheetIndex) == null) {
            ...
            addJAXBElement(stroke,
                           type.getEGShapeElements(),
                           CTStroke.class,
        ...
        addJAXBElement(fill,
                       shape.getEGShapeElements(),
                       org.docx4j.vml.CTFill.class,
                       "urn:schemas-microsoft-com:vml",
                       "fill",
                       "v");
        ...
        try {
            JAXBContext jc = JAXBContext.newInstance(org.docx4j.w14.ObjectFactory.class,
            ...
            JAXBElement<CTTextbox> box = jc.createUnmarshaller()
                    .unmarshal(new StreamSource(new StringReader(
                            "<textbox style=\"mso-direction-alt:auto\"><div style=\"text-align: left\" /></textbox>")),
                               CTTextbox.class);
            addJAXBElement(box.getValue(),
                           shape.getEGShapeElements(),
                           CTTextbox.class,
                           "urn:schemas-microsoft-com:vml",
                           "path",
            ...
            String clientData = "<ClientData ObjectType=\"Note\">" + "<MoveWithCells />" + "<SizeWithCells />"
                    + "<Anchor>" + position.toString() + "</Anchor>" + "<AutoFill>False</AutoFill>" + "<Row>"
                    + cellRange.startCellNumericRow() + "</Row>" + "<Column>" + cellRange.startCellNumericColumn()
                    + "</Column>" + "<Visible />" + "</ClientData>";

            JAXBElement<CTClientData> data = jc.createUnmarshaller()
                    .unmarshal(new StreamSource(new StringReader(clientData)), CTClientData.class);
            addJAXBElement(data.getValue(),
                           shape.getEGShapeElements(),
                           CTClientData.class,
    ...
    private <T> void addJAXBElement(T layout, List<Object> any, String ns, String local, String prefix,
            Class<T> clazz) {
        any.add(new JAXBElement<T>(new QName(ns, local, prefix), clazz, layout));
    }

    private <T> void addJAXBElement(T layout, List<JAXBElement<?>> any, Class<T> clazz, String ns, String local,
            String prefix) {
        any.add(new JAXBElement<T>(new QName(ns, local, prefix), clazz, layout));
    }
    ...
    private CTRElt createCommentChunk(String value, String rpr) {
        ...
        try {
            ...
            JAXBElement<CTRElt> element = jc.createUnmarshaller()
                    .unmarshal(new StreamSource(new StringReader("<r>" + rpr + "</r>")), CTRElt.class);
            r = element.getValue();
    ...
    public void createPageSetup(WorksheetBuilder worksheet, InputStream data, STOrientation orientation, int scale,
            int paperSize) throws Docx4JException {
        int sheetIndex = sheets.indexOf(worksheet.sheet);
        PrinterSettings settings = new PrinterSettings(
                new PartName("/xl/printerSettings/printerSettings" + sheetIndex + ".bin"));
    ...
    public void setPrintArea(WorksheetBuilder worksheet, char startCellCol, int startCellRow, char endCellCol,
            int endCellRow) {
        try {
            int sheetIndex = sheets.indexOf(worksheet.sheet);
            ...
            localSheet.setValue(getSheetName(sheetIndex) + "!$" + startCellCol + "$" + startCellRow + ":$" + endCellCol
                    + "$" + endCellRow);
     ...
     * 
     * The style with the given id is cloned but with the cell protection set to
     * unlocked.
     */
